...
(function() {
    ...
    class WebSocketClient {
        constructor(options = {}) {
            ...
            // Аутентификационные данные (устанавливаются перед вызовом connect)
            this.address = null;
            this.signature = null;
            this.nonce = null;
        }
        ...
        async connect() {
            ...
            if (!this.url) {
                // Собираем URL из текущего location
                const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
                this.url = `${protocol}//${window.location.host}/ws`;
            }
            // Добавляем аутентификацию в URL, если требуется
        ...
        _onClose(event) {
            ...
            this.isConnected = false;
            this.onDisconnect();
            if (this.shouldReconnect && event.code !== 1008) { // 1008 = Unauthorized, не переподключаемся
