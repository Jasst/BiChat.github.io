...
export default function ChatScreen() {
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity style={styles.item} onPress={() => openChat(item)}>
      ...
      <Text style={styles.time}>{item.last_time ? new Date(item.last_time * 1000).toLocaleTimeString() : ''}</Text>
    </TouchableOpacity>
  );

  if (loading) return <ActivityIndicator size="large" color="#ffffff" style={styles.loader} />;
...
const styles = StyleSheet.create({
  ...
  preview: { color: '#a4b0be', fontSize: 13, marginTop: 2 },
  time: { color: '#666', fontSize: 11 },
  empty: { color: '#a4b0be', textAlign: 'center', marginTop: 50 },
